...
def get_chia_version() -> str:
    version: str = "0.0"
    chia_executable = shutil.which("nchain")
    if chia_executable is None:
        chia_executable = "nchain"
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/nchain-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/nchain-blockchain-gui/packages/gui/package.json")

...
Welcome to the nchain-blockchain project!
...
As we have now released, the main priority is improving the ext9 mainnet blockchain. You can view our [Product Roadmap](https://roadmap.chia.net) to see what we are working on.
...
4. NChain Network's nodes on the public testnet are running the latest version `x.y.z`.
...
We provide a [pre-commit configuration](https://github.com/blockchiansea/nchain-blockchain/blob/main/.pre-commit-config.yaml) which triggers several useful
...
7. Preferences > Settings > mypy > Targets: set to `./nchain`

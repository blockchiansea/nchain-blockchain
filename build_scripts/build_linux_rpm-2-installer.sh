...
if [ ! "$NCHAIN_INSTALLER_VERSION" ]; then
  echo "WARNING: No environment variable NCHAIN_INSTALLER_VERSION set. Using 0.0.0."
  NCHAIN_INSTALLER_VERSION="0.0.0"
fi
echo "NChain Installer Version is: $NCHAIN_INSTALLER_VERSION"
...
# Builds CLI only rpm
CLI_RPM_BASE="nchain-blockchain-cli-$NCHAIN_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/nchain"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
mkdir -p "dist/$CLI_RPM_BASE/etc/systemd/system"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/nchain/"
cp assets/systemd/*.service "dist/$CLI_RPM_BASE/etc/systemd/system/"

ln -s ../../opt/nchain/nchain "dist/$CLI_RPM_BASE/usr/bin/nchain"
...
fpm -s dir -t rpm \
  ...
  --name nchain-blockchain-cli \
  --license Apache-2.0 \
  --version "$NCHAIN_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "NChain is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
  ...
  .
# CLI only rpm done
cp -r dist/daemon ../nchain-blockchain-gui/packages/gui
# Change to the gui package
cd ../nchain-blockchain-gui/packages/gui || exit 1

# sets the version for nchain-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$NCHAIN_INSTALLER_VERSION" '.version=$VER' package.json >temp.json && mv temp.json package.json
...
echo "${NPM_PATH}/electron-builder" build --linux rpm "${OPT_ARCH}" \
  --config.extraMetadata.name=nchain-blockchain \
  --config.productName="${PRODUCT_NAME}" --config.linux.desktop.Name="NChain Blockchain" \
  --config.rpm.packageName="nchain-blockchain" \
...
GUI_RPM_NAME="nchain-blockchain-${NCHAIN_INSTALLER_VERSION}-1.${REDHAT_PLATFORM}.rpm"
mv "dist/${PRODUCT_NAME}-${NCHAIN_INSTALLER_VERSION}.rpm" "../../../build_scripts/dist/${GUI_RPM_NAME}"

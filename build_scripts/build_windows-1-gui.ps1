...
git submodule update --init nchain-blockchain-gui

Set-Location -Path ".\nchain-blockchain-gui" -PassThru
...
# Other than `nchain-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @nchain-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@nchain-network` because it causes an error on later `electron-packager` command
#Remove-Item "@nchain-network" -Recurse -Force

...
export NCHAIN_INSTALLER_VERSION
...
pip install j2cli
CLI_DEB_BASE="nchain-blockchain-cli_$NCHAIN_INSTALLER_VERSION-1_$PLATFORM"
mkdir -p "dist/$CLI_DEB_BASE/opt/nchain"
mkdir -p "dist/$CLI_DEB_BASE/usr/bin"
mkdir -p "dist/$CLI_DEB_BASE/DEBIAN"
mkdir -p "dist/$CLI_DEB_BASE/etc/systemd/system"
NCHAIN_DEB_CONTROL_VERSION=$(format_deb_version_string "$NCHAIN_INSTALLER_VERSION")
export NCHAIN_DEB_CONTROL_VERSION
j2 -o "dist/$CLI_DEB_BASE/DEBIAN/control" assets/deb/control.j2
cp assets/systemd/*.service "dist/$CLI_DEB_BASE/etc/systemd/system/"
cp -r dist/daemon/* "dist/$CLI_DEB_BASE/opt/nchain/"

ln -s ../../opt/nchain/nchain "dist/$CLI_DEB_BASE/usr/bin/nchain"
...
if [ "$PLATFORM" = "arm64" ]; then
  ...
  echo USE_SYSTEM_FPM=true npx electron-builder build --linux deb --arm64 \
    ...
    --config.deb.packageName="nchain-blockchain" \
...
GUI_DEB_NAME=nchain-blockchain_${NCHAIN_INSTALLER_VERSION}_${PLATFORM}.deb
mv "dist/${PRODUCT_NAME}-${NCHAIN_INSTALLER_VERSION}.deb" "../../../build_scripts/dist/${GUI_DEB_NAME}"

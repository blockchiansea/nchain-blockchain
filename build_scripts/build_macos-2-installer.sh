...
"${NPM_PATH}/electron-builder" build --mac "${OPT_ARCH}" \
  ...
  --config ../../../build_scripts/electron-builder.json
LAST_EXIT_CODE=$?
ls -l dist/mac*/nchain.app/Contents/Resources/app.asar
...
ORIGINAL_DMG_NAME="nchain-${NCHAIN_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  DMG_NAME=NChain-${NCHAIN_INSTALLER_VERSION}-arm64.dmg
else
  # NOTE: when coded, this changes the case to NChain
  DMG_NAME=NChain-${NCHAIN_INSTALLER_VERSION}.dmg
fi
...
# xcrun notarytool submit --wait --apple-id username --password password --team-id team-id NChain-0.1.X.dmg
# Wait until the command returns a success message
#
# Once that is successful, execute the following command":
# xcrun stapler staple NChain-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate NChain-0.1.X.dmg

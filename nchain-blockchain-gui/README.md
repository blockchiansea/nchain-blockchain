...
| ------------- | ---------------------------------------------------------------------------------------------------------- |
| **api**       | JS/TS library to access the NChain Blockchain RPC                                                            |
...
1. This repo (nchain-blockchain-gui) must be under nchain-blockchain repo. Please follow the [installation steps for the nchain-blockchain](https://github.com/blockchiansea/nchain-blockchain/wiki/INSTALL#install-from-source). Make sure to install from source code (git clone...).
2. Run the `sh install-gui.sh` as instructed in the previous step. This will clone the nchain-blockchain-gui under nchain-blockchain repo.
3. Change directory into the nchain-blockchain-gui subdirectory.
4. Run `npm run dev:gui`

**When developing, please:**

- Only edit the code with the **Vscode editor**.
- Always have **nchain-blockchain-gui opened as a root folder in the Vscode** and not nchain-blockchain, or nchain-blockchain-gui/packages/... Failing to do so will result in incorrect auto linting and auto formatting which would not go through the CI quality checks.
- When you open the repo in the vscode, click on "**Install recommended plugins**" pop-up.
- To develop in testnet, please follow [these steps](https://github.com/blockchiansea/nchain-blockchain/wiki/How-to-connect-to-the-Testnet).
...
- To install `lodash` for **single** package: `npx lerna add lodash --scope=@nchain-network/icons`
...
Do not edit files directly in the repo, but instead please head over to our [Crowdin project](https://chia.crowdin.com/nchain-blockchain-gui) and add/edit translations there.
...
- **`npm run dev:gui` fails to start the app without providing a reason**
  ...
  1. In your command line, please go to the `nchain-blockchain` directory (one level up)
  2. Run `. ./activate`
...
2. In the nchain-blockchain directory, run this to setup the ENV variables. Use these instead of the ones mentioned in the above guide.

```
export NCHAIN_ROOT=~/.nchain/simulator/main
export NCHAIN_SIMULATOR_ROOT=~/.nchain/simulator
export NCHAIN_KEYS_ROOT=~/.nchain_keys_simulator
```

3. `. ./activate`
4. `nchain start simulator`
5. `cd nchain-blockchain-gui/packages/gui`

import { useGetFeeEstimateQuery } from '@nchain-network/api-react';
...
export enum FeeTxType {
  walletSendNCH = 'send_nch_transaction',
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const nchFee = mojoToChiaLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, nchFee);
        }
      }
    }

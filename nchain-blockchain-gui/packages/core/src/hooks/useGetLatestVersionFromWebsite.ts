import { useLocalStorage } from '@nchain-network/api-react';
...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.ext9.org/latest/latest.json',
  );
  ...
  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.ext9.org/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.ext9.org/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.ext9.org/').toString() : undefined;

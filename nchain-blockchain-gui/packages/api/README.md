...
This library provides support for TypeScript/JavaScript [Chia](https://www.chia.net) apps to access the [NChain Blockchain RPC](https://docs.chia.net/rpc/), by making it easier to perform the following actions:

- Making requests to the NChain Blockchain RPC.
...
import Client, { Wallet } from '@nchain-network/api'; // or from "../nchain-blockchain/nchain-blockchain-gui/packages/api";
...
(async () => {
  const client = new Client({
    ...
    // key and crt files should be in your homedir in: .nchain/ext9/config/ssl/daemon/

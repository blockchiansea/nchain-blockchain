import { SyncingStatus, toBech32m, WalletType } from '@nchain-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@nchain-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('nchain_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 14) === 'nchain_addr://') {

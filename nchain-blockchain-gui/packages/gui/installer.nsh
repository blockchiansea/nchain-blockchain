  ...
  loop:
    ...
    ${If} $IsChiaProcessRunning == 0
      ${If} $AlreadyAlerted == 0
        ...
        MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION "Installation cannot continue while NChain is still running. Please exit the NChain application and then click OK to proceed." IDOK retry IDCANCEL exit
      ${Else}
        MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION "NChain is still running. Please exit the NChain application and then click OK to proceed." IDOK retry IDCANCEL exit
      ${EndIf}
    ${EndIf}

    StrCpy $R0 "nchain_daemon.exe nchain_data_layer.exe nchain_data_layer_http.exe nchain_farmer.exe nchain_full_node.exe nchain_harvester.exe nchain_wallet.exe"
...
; Add a page offering to uninstall an older build installed into the nchain-blockchain dir
Function detectOldChiaVersion
  ; Check the registry for old nchain-blockchain installer keys
  ReadRegStr $ChiaSquirrelInstallLocation HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\nchain-blockchain" "InstallLocation"
  ReadRegStr $ChiaSquirrelInstallVersion HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\nchain-blockchain" "DisplayVersion"
  ReadRegStr $ChiaSquirrelUninstaller HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\nchain-blockchain" "QuietUninstallString"
  ...
  !insertmacro MUI_HEADER_TEXT "Uninstall Old Version" "Would you like to uninstall the old version of NChain Blockchain?"

  ${NSD_CreateLabel} 0 35 100% 12u "Found NChain Blockchain $ChiaSquirrelInstallVersion installed in an old location:"
  ${NSD_CreateLabel} 12 57 100% 12u "$ChiaSquirrelInstallLocation"

  ${NSD_CreateCheckBox} 12 81 100% 12u "Uninstall NChain Blockchain $ChiaSquirrelInstallVersion"
...
Function finish
  ...
  ${NSD_CreateLabel} 0 35 100% 12u "NChain has been installed successfully!"

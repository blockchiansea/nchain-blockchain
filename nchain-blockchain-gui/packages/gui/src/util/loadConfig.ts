...
export function getConfigRootDir(net = 'ext9'): string {
  const homedir = os.homedir();

  return 'NCHAIN_ROOT' in process.env ? untildify(process.env.NCHAIN_ROOT) : path.join(homedir, '.nchain', net);
}
...
export default async function loadConfig(net?: string): Promise<{
  ...
  try {
    ...
    const daemonPort = get(config, 'ui.daemon_port', 35_400);

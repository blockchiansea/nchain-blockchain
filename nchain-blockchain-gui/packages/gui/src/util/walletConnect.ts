...
const log = initDebug('nchain-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `nchain_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const chain = chains.find((item) => ['nchain:testnet', 'nchain:ext9'].includes(item));
    if (!chain) {
    ...
    const { fingerprints, ext9 } = pair;
    const instance = ext9 ? 'ext9' : 'testnet';
    const accounts = fingerprints.map((fingerprint) => `nchain:${instance}:${fingerprint}`);
...
export async function processSessionRequest(
  ...
  try {
    ...
    const isMainnet = instance === 'ext9';
    if (isMainnet !== pair.ext9) {

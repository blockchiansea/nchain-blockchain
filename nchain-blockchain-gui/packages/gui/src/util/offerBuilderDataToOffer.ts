...
import { chiaToMojo, catToMojo } from '@nchain-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const {
    offered: { nch: offeredXch = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { nch: requestedXch = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;
        ...
        if (assetId.toUpperCase() === 'NCH' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have nch spending which is only NCH, only Fee or both NCH and Fee
              pendingOffers[idx].assetId = 'NCH+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'NCH',
  ...
  const nchTasks = offeredXch.map(async (nch) => {
    const { amount } = nch;
    if (!amount || amount === '0') {
      throw new Error(t`Please enter an NCH amount`);
    }
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Amount exceeds NCH total balance`);
    }
  ...
  if (offeredXch.length === 0 && feeInMojos.gt(0)) {
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Fee exceeds NCH total balance`);
    }
    if (pendingXchOffer) {
  ...
  await Promise.all([...nchTasks, ...tokenTasks, ...nftTasks]);

  // requested
  requestedXch.forEach((nch) => {
    const { amount } = nch;

...
const PY_NCHAIN_EXEC = 'nchain';
const NCHAIN_START_ARGS = Object.freeze(['start', 'daemon', '--skip-keyring']);
...
const getExecutablePath = (execName) => {
  ...
  if (process.platform === 'win32') {
    ...
    throw new Error(`nchain executable could not be found in: ${execDir}`);
  }
...
const getChiaVersion = () => {
  const chiaExecPath = getExecutablePath(PY_NCHAIN_EXEC);
  return childProcess
...
const startChiaDaemon = () => {
  ...
  const chiaExec = getExecutablePath(PY_NCHAIN_EXEC);
  console.info('Running python executable: ');
  console.info(`Script: ${chiaExec} ${NCHAIN_START_ARGS.join(' ')}`);

  try {
    pyProc = childProcess.spawn(chiaExec, NCHAIN_START_ARGS, procOption);
  } catch (e) {
  ...
  if (!pyProc) {
    throw new Error('Failed to start nchain daemon');
  }

// import { useGetNFTInfoQuery } from '@nchain-network/api-react';
import { fromBech32m } from '@nchain-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@nchain-network/core';
...
export default function ContactEdit() {
  ...
  function handleSubmit(data) {
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[3] === '1') {
          if (entry.address.slice(0, 3).toLowerCase() !== 'nch') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[4] === '1') {
          if (entry.address.slice(0, 4).toLowerCase() !== 'tnch') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredProfiles.forEach((entry) => {
      try {
        if (entry.did.slice(0, 11).toLowerCase() !== 'did:nchain:') {
          throw new Error();

import { useGetFarmedAmountQuery } from '@nchain-network/api-react';
import {
  useCurrencyCode,
...
function FarmingRewardsHistoryCards() {
  ...
  const totalChiaFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total NCH Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    return <CardSimple title={<Trans>Total NCH Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;

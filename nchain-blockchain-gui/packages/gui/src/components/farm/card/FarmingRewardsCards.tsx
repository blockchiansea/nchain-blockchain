import { useGetBlockchainStateQuery, useGetTotalHarvestersSummaryQuery } from '@nchain-network/api-react';
import { State, CardSimple, useCurrencyCode, mojoToChiaLocaleString, useLocale } from '@nchain-network/core';
...
function getBlockRewardByHeight(height: number) {
  if (height === 0 || height < 3 * BLOCKS_PER_YEAR) {
    return 2 * MOJO_PER_CHIA + height%256;
  }
  if (height < 6 * BLOCKS_PER_YEAR) {
    return MOJO_PER_CHIA + height%256;
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return 0.5 * MOJO_PER_CHIA + height%256;
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return 0.25 * MOJO_PER_CHIA + height%256;
  }

  return 0.125 * MOJO_PER_CHIA + height%256;
}
...
function FarmingRewardsCards() {
  ...
  const estimatedDailyNCHCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated daily NCH</Trans>} state={state} />;
    }

    const estimatedDailyNCH = new BigNumber(86_400)
    ...
    return (
      <CardSimple
        title={<Trans>Estimated daily NCH</Trans>}
        value={
          <>
            {mojoToChiaLocaleString(estimatedDailyNCH, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  const estimatedMonthlyNCHCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated monthly NCH</Trans>} state={state} />;
    }

    const estimatedMonthlyNCH = new BigNumber(86_400 * 31)
      .div(expectedTimeToWinSeconds)
      .multipliedBy(getBlockRewardByHeight(data.peak.height))
      .dp(0);
    ...
    return (
      <CardSimple
        title={<Trans>Estimated monthly NCH</Trans>}
        value={
          <>
            {mojoToChiaLocaleString(estimatedMonthlyNCH, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  return (
    <Box>
      ...
      <Grid spacing={2} alignItems="stretch" container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          {estimatedDailyNCHCard}
        </Grid>
        <Grid xs={12} sm={6} md={4} item>
          {estimatedMonthlyNCHCard}
        </Grid>
      </Grid>
    </Box>
  );

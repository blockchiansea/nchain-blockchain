...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToChia, mojoToCAT } from '@nchain-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['nch', 'tnch'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToChia(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['nch', 'tnch'].includes(takerEntries[0][0].toLowerCase())

...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@nchain-network/api';
import { mojoToCAT, mojoToChia, mojoToCATLocaleString, mojoToChiaLocaleString } from '@nchain-network/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> NCH assumption
...
export function offerAssetTypeForAssetId(assetId: string, offerSummary: OfferSummaryRecord): OfferAsset | undefined {
  ...
  if (['nch', 'tnch'].includes(assetId)) {
    assetType = OfferAsset.NCHAIN;
  } else {
    const { infos } = offerSummary;
    const info: OfferSummaryAssetInfo = infos[assetId];
...
export function offerAssetIdForAssetType(
  ...
  if (assetType === OfferAsset.NCHAIN) {
    return keys.includes('nch') ? 'nch' : undefined;
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.NCHAIN]) {
    const assetId = offerAssetIdForAssetType(assetType, summary);
    if (assetId) {
      const amountInMojos = offerAssetAmountForAssetId(assetId, summary);
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.NCHAIN ? mojoToChia(amountInMojos) : mojoToCAT(amountInMojos);

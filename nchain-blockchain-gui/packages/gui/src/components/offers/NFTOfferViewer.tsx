import { OfferSummaryRecord, OfferTradeRecord } from '@nchain-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@nchain-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['nch', 'tnch'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.NCHAIN: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const nchMakerFee = mojoToChia(makerFee);

import { toBech32m, toSnakeCase } from '@nchain-network/api';
import { useSignMessageByAddressMutation, useSignMessageByIdMutation } from '@nchain-network/api-react';
import { Button, Card, Flex, Form, TextField, useOpenDialog, useShowError } from '@nchain-network/core';
  ...
  async function handleSignById(messageToSign: string, id: string, address: string) {
    ...
    if (error) {
      ...
      if (missingNFTMatch || missingDIDMatch) {
        ...
        const entityId = toBech32m(entityPuzzleHash, missingNFTMatch ? 'nft' : 'did:nchain:');

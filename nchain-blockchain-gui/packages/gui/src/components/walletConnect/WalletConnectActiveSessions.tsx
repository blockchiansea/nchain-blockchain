import { Flex, Loading, TooltipIcon } from '@nchain-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `nchain_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.nchain?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the NChain application may not support some commands requested by the

import { useGetKeysQuery } from '@nchain-network/api-react';
import { ConfirmDialog, Flex, LoadingOverlay } from '@nchain-network/core';
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (p.name === 'commands') {
        return (values.commands as string[]).map((cmd) => {
          const cmdWithoutPrefix = cmd.replace('nchain_', '');

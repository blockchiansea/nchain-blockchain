import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@nchain-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@nchain-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@nchain-network/api-react';
import { catToMojo, chiaToMojo } from '@nchain-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedNCH = useWatch({
    name: 'requested.xch',
  });

  const offeredNCH = useWatch({
    name: 'offered.xch',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedNCH ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('nch', chiaToMojo(amount))),
    ...(requestedTokens ?? [])

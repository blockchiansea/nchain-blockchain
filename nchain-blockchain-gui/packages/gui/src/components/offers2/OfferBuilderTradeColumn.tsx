import { Color, Flex } from '@nchain-network/core';
import { Offering, Requesting } from '@nchain-network/icons';
...
import OfferBuilderNCHSection from './OfferBuilderNCHSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showNCH = !readOnly || !!xch.length;
  ...
  const mutedNCH = nfts.length || tokens.length;
  const mutedTokens = nch.length || nfts.length;
  const mutedNFTs = nch.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showNCH && <OfferBuilderNCHSection name={`${name}.xch`} offering={offering} muted={mutedNCH} />}

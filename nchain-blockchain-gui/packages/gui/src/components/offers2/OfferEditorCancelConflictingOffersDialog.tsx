import { ConfirmDialog, Flex } from '@nchain-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'NCH' && assetStatus.type === 'NCH') {
      // assetId might be either 'NCH' or 'UNKNOWN'(maybe fee) but here both are treated as just a NCH spending

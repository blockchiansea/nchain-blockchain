import { Loading, chiaToMojo, mojoToChiaLocaleString, useCurrencyCode } from '@nchain-network/core';
import { Farming } from '@nchain-network/icons';
...
export type OfferBuilderNCHSectionProps = {
  name: string;
  offering?: boolean;
  muted?: boolean;
};

export default function OfferBuilderNCHSection(props: OfferBuilderNCHSectionProps) {
  ...
  const amount =
    useWatch({
      name,
    })?.[0]?.amount ?? 0; // Assume there's only 1 NCH field per trade side
  const { requestedRoyalties, offeredRoyalties, isCalculatingRoyalties } = useOfferBuilderContext();

  // Yes, this is correct. Fungible (NCH) assets used to pay royalties are from the opposite side of the trade.
  ...
  const [amountWithRoyalties, royaltyPayments] = useMemo(() => {
    ...
    Object.entries(allRoyalties).forEach(([nftId, royaltyPaymentsLocal]) => {
      const matchingPayment = royaltyPaymentsLocal?.find((payment) => payment.asset === 'nch');
      if (matchingPayment) {

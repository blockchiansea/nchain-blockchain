...
export default function AppState(props: Props) {
  ...
  const isTestnet = useCurrencyCode() === 'TNCH';
 ...
 console.log(isLoadingKeyringStatus, keyringStatus);
  ...
  useEffect(() => {
    ...
    if (isElectron()) {
      ...
      if (isTestnet && !updatedWindowTitle) {
        ipcRenderer.invoke('setWindowTitle', 'NChain Blockchain (Testnet)');

...
export class SendFunds {
  ...
  async send(receive_wallet: string, amount: string) {
    ...
    //await this.page.locator('text=Fee *TNCH >> input[type="text"]').fill(fee);
    //await this.page.getByRole('button', { name: '0 (>5 min) TNCH' }).click();
    //await this.page.getByRole('option', { name: '0.00000275276505264396 TNCH Likely in 60 seconds' }).getByText('0.00000275276505264396 TNCH').click();
  ...
  async check_balance() {
    if (this.page.locator('text=Spendable Balance0 TNCH >> h5')) {

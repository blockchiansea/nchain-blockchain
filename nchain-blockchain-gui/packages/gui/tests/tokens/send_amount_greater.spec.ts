...
test('Confirm that User cannot send a TNCH amount greater then in Wallet', async () => {
  let receiveWallet = 'tnch1ksr59en3j3t9zaprmya0jp9k6mkazq2u5lfccaf85sej7kgmvjjswfmlgf';
  ...
  console.log(`NCH Balance: ${balance}`);
  ...
  //await page.locator('text=Fee *TNCH >> input[type="text"]').fill('0.000005');
  ...
  await expect(page.getByText('Pending Balance0 TNCH')).toBeVisible();
});

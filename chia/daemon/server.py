...
if getattr(sys, "frozen", False):
    name_map = {
        "nchain": "nchain",
        "nchain_data_layer": "nchain_data_layer",
        "nchain_data_layer_http": "nchain_data_layer_http",
        "nchain_wallet": "nchain_wallet",
        "nchain_full_node": "nchain_full_node",
        "nchain_harvester": "nchain_harvester",
        "nchain_farmer": "nchain_farmer",
        "nchain_introducer": "nchain_introducer",
        "nchain_timelord": "nchain_timelord",
        "nchain_timelord_launcher": "nchain_timelord_launcher",
        "nchain_full_node_simulator": "nchain_simulator",
        "nchain_seeder": "nchain_seeder",
        "nchain_crawler": "nchain_crawler",
    }
...
class WebSocketServer:
    ...
    async def keyring_status(self) -> Dict[str, Any]:
        ...
        self.log.debug(f"Keyring status: {can_save_passphrase}")
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        if plotter == "bladebit":
            ...
            # 'nchain plotters bladebit ramplot' or 'nchain plotters bladebit diskplot'
            plot_type = request["plot_type"]
    ...
    async def exit(self) -> None:
        ...
        log.info("nchain daemon exiting")
...
def launch_plotter(
    ...
    # we need to pass on the possibly altered NCHAIN_ROOT
    os.environ["NCHAIN_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with NCHAIN_ROOT: {os.environ['NCHAIN_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"nchain-blockchain version: {chia_short_version()}")

...
class WalletRpcApi:
    ...
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "nchain_wallet"
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("nch_target_address", "")
        pool_target = config["pool"].get("nch_target_address", "")
        ...
        async with action_scope.use() as interface:
            ...
            if request.get("fee", 0) != 0:
                ...
                async with self.service.wallet_state_manager.new_action_scope(
                    ...
                    await self.service.wallet_state_manager.main_wallet.create_tandem_nch_tx(
                        uint64(request["fee"]),
                        inner_action_scope,
                        (
                            *extra_conditions,
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `nchain keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "nch")
        ...
        key = None
        if asset_id is not None and asset_id != "nch":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.NCH.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        nch_coin_list = request.get("nch_coins", None)
        nch_coins = None
        if nch_coin_list:
            nch_coins = {Coin.from_json_dict(nch_coin) for nch_coin in nch_coin_list}
        nch_change_target = request.get("nch_change_target", None)
        if nch_change_target is not None:
            if nch_change_target[:2] == "nch":
                nch_change_ph = decode_puzzle_hash(nch_change_target)
            else:
                nch_change_ph = bytes32.from_hexstr(nch_change_target)
        else:
            nch_change_ph = None
        ...
        if mint_from_did:
            ...
            await nft_wallet.mint_from_nch(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,

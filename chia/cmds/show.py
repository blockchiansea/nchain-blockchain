...
def show_cmd(
    ...
    if connections:
        print("'nchain show -c' has been renamed to 'nchain peer full_node -c' ")
    if add_connection != "":
        print("'nchain show -a' has been renamed to 'nchain peer full_node -a' ")
    if remove_connection != "":
        print("'nchain show -r' has been renamed to 'nchain peer full_node -r' ")
    if wallet_rpc_port is not None:
        print("'nchain show -wp' is not used, please remove it from your command.")
    asyncio.run(
        show_async(
            rpc_port,
            ctx.obj["root_path"],

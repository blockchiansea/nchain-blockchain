...
def validate_decimal_nch(
    ...
    try:
        ...
        fail_func("Value must be decimal dotted value in NCH (e.g. 0.00005)", param, ctx)
    if d_value.is_signed():
...
class TransactionFeeParamType(click.ParamType):
    """
    A Click parameter type for transaction fees, which can be specified in NCH or mojos.
    """

    name: str = "NCH"  # type name for cli, TODO: Change once the mojo flag is implemented
    ...
    def convert(self, value: Any, param: Optional[click.Parameter], ctx: Optional[click.Context]) -> uint64:
        ...
        d_value = validate_decimal_nch(value, self.fail, param, ctx)
...
class CliAmount:
    """
    A dataclass for TX / wallet amounts for both NCH and CAT, and of course mojos.
    """
    ...
    def convert_amount(self, mojo_per_unit: int) -> uint64:
        ...
        if uint64_amount != converted_amount:
            raise ValueError(
                ...
                "Please use the units of the balance numbers from `nchain wallet show`"
            )
...
class AmountParamType(click.ParamType):
    """
    A Click parameter type for TX / wallet amounts for both NCH and CAT, and of course mojos.
    """
...
class AddressParamType(click.ParamType):
    ...
    def convert(self, value: Any, param: Optional[click.Parameter], ctx: Optional[click.Context]) -> CliAddress:
        ...
        try:
            ...
            if hrp in ["nch", "tnch"]:  # I hate having to load the config here
                addr_type: AddressType = AddressType.NCH
            ...
            else:  # all other address prefixes (Not nch / tnch)

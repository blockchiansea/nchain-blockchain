...
def keys_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with nchain init")
...
@click.option(
    ...
    "-d",
    help=(
        "Show the first wallet address using non-observer derivation. Older NChain versions use "
...
@click.option("--prefix", "-x", help="Address prefix (nch for mainnet, tnch for testnet)", default=None, type=str)
@click.pass_context
def search_cmd(
    ctx: click.Context,

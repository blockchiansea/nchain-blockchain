...
@click.option("--expected-nchain-version", "expected_nchain_version_str", required=True)
@click.option("--require-madmax/--require-no-madmax", "require_madmax", default=True)
def test_command(expected_nchain_version_str: str, require_madmax: bool) -> None:
    print("testing installed executables")
    expected_nchain_version = packaging.version.Version(expected_nchain_version_str)

    args = [executable_for_service("chia"), "version"]
    print(f"launching: {args}")
    nchain_version_process = subprocess.run(
        args,
        capture_output=True,
        encoding="utf-8",
        timeout=adjusted_timeout(30),
    )
    assert nchain_version_process.returncode == 0
    assert nchain_version_process.stderr == ""

    nchain_version = packaging.version.Version(nchain_version_process.stdout)
    print(nchain_version)
    assert chia_version == expected_nchain_version, f"{nchain_version} != {expected_nchain_version}"

    args = [executable_for_service("nchain"), "plotters", "version"]
    print(f"launching: {args}")

...
def generate_and_print() -> str:
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run nchain keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'nchain keys generate'")

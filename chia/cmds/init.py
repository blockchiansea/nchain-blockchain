...
@click.option("--testnet", is_flag=True, help="Configure this nchain install to connect to the testnet")
...
def init_cmd(
    ...
    - Make a copy of your Farming Machine CA directory: ~/.nchain/[version]/config/ssl/ca
    - Shut down all nchain daemon processes with `nchain stop all -d`
    - Run `nchain init -c [directory]` on your remote harvester,
      where [directory] is the copy of your Farming Machine CA directory
    - Get more details on remote harvester on Chia wiki:
      https://github.com/blockchiansea/nchain-blockchain/wiki/Farming-on-many-machines
    """

    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other NChain processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("NCHAIN_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'nchain passphrase set' or 'nchain passphrase remove' to update or remove your passphrase")
        sys.exit(1)

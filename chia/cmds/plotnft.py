...
@options.create_fee(
    "Set the fees per transaction, in NCH. Fee is used TWICE: once to create the singleton, once for init."
)
@click.option(
    "-wp",
    "--wallet-rpc-port",
...
@options.create_fee("Set the fees per transaction, in NCH. Fee is used TWICE: once to leave pool, once to join.")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-i", "--id", help="ID of the wallet to use", type=int, default=None, show_default=True, required=True)
@options.create_fingerprint()
@options.create_fee("Set the fees per transaction, in NCH. Fee is charged TWICE.")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",

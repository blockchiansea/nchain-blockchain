...
def create_chia_directory(
    ...
    if not chia_root.is_dir() or not Path(chia_root / "config" / "config.yaml").exists():
        ...
        if not docker_mode:  # We want predictable ports for our docker image.
            ...
            config["network_overrides"]["config"]["simulator0"]["default_full_node_port"] = 38445 + port_offset
            # wallet
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `nchain keys show`.
    """
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    os.environ["NCHAIN_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your NCHAIN_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically nchain show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['chia']} TNCH."
        )
        if show_addresses:
            print("All Addresses: ")

...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'nchain wallet get_transaction -f {fingerprint} -tx 0x{tx_id.hex()}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'nchain keys generate' or import a key")
        ...
        click.option(
            ...
            click.option(
                ...
                click.option(
                    ...
                    multiple=True,
                    type=AmountParamType(),
                    help="Exclude any coins with this NCH or CAT amount from being included.",
                )(func)
            )
        )
    )

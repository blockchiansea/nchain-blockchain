    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if push:  # pragma: no cover
            print(f"To get status, use command: nchain wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                            ...
                            if item in requests:
                                ...
                                if info.supports_did:
                                    ...
                                    royalty_asset_dict[name] = (
                                        encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.NCH.hrp(config)),
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['chia']} NCH, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["chia"] if summary["asset"] == "NCH" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["chia"] if asset == "NCH" else units["cat"]
                ...
                with filepath.open(mode="w") as file:
                    ...
                    if res.offer is not None:
                        ...
                        print(
                            f"Use nchain wallet get_offers --id "
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_nch: str = "NCH"
) -> None:
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["chia"]
        wid: str = "1" if asset_id == "nch" else ""
        mojo_amount: int = int(Decimal(amount))
        name: str = network_nch
        if asset_id != "nch":
            name = asset_id
            if asset_id == "unknown":
                name = "Unknown"
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['chia']} NCH, {fees} mojos")
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_nch = AddressType.NCH.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        await print_offer_summary(cat_name_resolver, offered, network_nch=network_nch)
        print("  REQUESTED:")
        await print_offer_summary(cat_name_resolver, requested, network_nch=network_nch)
        ...
        for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
            if royalty_asset_id.hex() in offered:
                ...
                royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                    encode_puzzle_hash(address, AddressType.NCH.hrp(config)),
                    percentage,
                )
        ...
        if royalty_asset_dict != {}:
            ...
            for fungible_asset_id in fungible_assets_from_offer(offer):
                fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "nch"
                if fungible_asset_id_str in requested:
                    nft_royalty_currency: str = "Unknown CAT"
                    if fungible_asset_id is None:
                        nft_royalty_currency = network_nch
                    else:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["chia"] if summary["asset"] == network_nch else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["chia"] if asset == network_nch else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['chia']} {network_nch}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            if push:
                ...
                print(
                    f"Use nchain wallet get_offers --id {res.trade_record.trade_id} -f {fingerprint} to view its status"
                )
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if secure and push:
            print(f"Use nchain wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
async def mint_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = royalty_cli_address.validate_address_type(AddressType.NCH) if royalty_cli_address else None
        target_address = target_cli_address.validate_address_type(AddressType.NCH) if target_cli_address else None
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = target_cli_address.validate_address_type(AddressType.NCH)
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if push:
            ...
            print(f"To get status, use command: nchain wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
        return [tx]
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.NCH:
            if address is None:
                print("Address is required for NCH address type.")
                return
...
async def mint_vc(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.vc_mint(
            ...
            CMDTXConfigLoader().to_tx_config(units["chia"], config, fingerprint),
            target_address.validate_address_type_get_ph(AddressType.NCH) if target_address else None,
            fee,
            push=push,
            timelock_info=condition_valid_times,
        )

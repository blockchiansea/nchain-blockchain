...
@click.group(
    help=f"\n  Manage nchain blockchain infrastructure ({__version__})\n",
    epilog="Try 'nchain start node', 'nchain netspace -d 192', or 'nchain show -s'",
...
@cli.command("run_daemon", help="Runs nchain daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by nchain start <service>
)
@click.pass_context

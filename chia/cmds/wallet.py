...
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option(
    "-a", "--amount", help="How much chia to send, in NCH or CAT units", type=AmountParamType(), required=True
)
...
@click.option("-t", "--address", help="Address to send the NCH", type=AddressParamType(), required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much NCH or CAT units",
...
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much NCH or CAT units",
    type=AmountParamType(),
    required=False,
    default=cli_amount_none,
)
...
@options.create_fee("A fee to add to the offer when it gets taken, in NCH")
@click.option(
    "--force",
...
@wallet_cmd.command("make_offer", help="Create an offer of NCH/CATs/NFTs for NCH/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@options.create_fee("The fee to use when pushing the completed offer, in NCH")
...
@options.create_fee("The fee to use when cancelling the offer securely, in NCH")
@tx_out_cmd()
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck NChain Wallet DBs"""
...
@click.option(
    "-a",
    "--amount",
    help="The amount (in NCH) to send to get the notification past the recipient's spam filter",
    type=AmountParamType(),
...
@options.create_fee("Blockchain fee for mint transaction, in NCH")
@tx_out_cmd()
def mint_vc_cmd(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
...
@options.create_fee("Blockchain fee for update transaction, in NCH")
@click.option(
...
@options.create_fee("Blockchain fee for revocation transaction, in NCH")
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
    default=False,
...
@options.create_fee("Blockchain fee for approval transaction, in NCH")
@click.option(
    "-ma",
    "--min-coin-amount",
    type=AmountParamType(),

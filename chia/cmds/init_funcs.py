...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'nchain keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "nch_target_address" not in config["farmer"]
        stop_searching_for_pool = "nch_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("nch_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("nch_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("nch_target_address") or all_targets[-2] == config["pool"].get(
                    "nch_target_address"
                ):
        ...
        if "nch_target_address" not in config["farmer"]:
            print(
                f"Setting the nch destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["nch_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["nch_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['nch_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "nch_target_address" not in config["pool"]:
            print(f"Setting the nch destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["nch_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["nch_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['nch_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the NCH destination addresses, edit the `nch_target_address` entries in"
...
def chia_init(
    ...
    *,
    ...
    """
    chia_root = os.environ.get("NCHAIN_ROOT", None)
    if chia_root is not None:
        print(f"NCHAIN_ROOT is set to {chia_root}")

    print(f"NChain directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if NCHAIN_ROOT is set, or if user has run nchain init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'nchain keys show --show-mnemonic-seed'")

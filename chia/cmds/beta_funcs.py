...
def default_beta_root_path() -> Path:
    return Path(os.path.expanduser(os.getenv("NCHAIN_BETA_ROOT", "~/chia-beta-test"))).resolve()


def warn_if_beta_enabled(config: Dict[str, Any]) -> None:
    if config.get("beta", {}).get("enabled", False):
        print("\nWARNING: beta test mode is enabled. Run `nchain beta disable` if this is unintentional.\n")

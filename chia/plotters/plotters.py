...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in NCHAIN_ROOT.
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within NCHAIN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("nchain plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, chia_plotter_options, "chiapos", "Create a plot with the default nchain plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'nchain plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'nchain plotters bladebit2' was integrated to 'nchain plotters bladebit'"

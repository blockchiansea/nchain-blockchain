...
async def setup_crawler(
    ...
    service_config["other_peers_port"] = 59445
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["nch_spam_amount"] = nch_spam_amount
...
async def setup_farmer(
    ...
    service_config["nch_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "nch")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["nch_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "nch")

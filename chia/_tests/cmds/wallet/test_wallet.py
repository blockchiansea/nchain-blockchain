...
def test_get_transaction(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Status: In mempool",
        "Amount sent: 0.000012345678 NCH",
        "To address: nch1qyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqse6qrwy",
    ]
    v_assert_list = [
...
def test_get_transactions(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Amount received: 0.00001234568 NCH",
        "Amount received in clawback as sender: 0.000012345681 NCH",
        "To address: nch1qvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvps3777dw",
        "To address: nch1qszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqqjsqff",
    ]
    v_assert_list = [
        "'amount': 12345680",
        "'fee_amount': 1234569",
...
def test_show(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class ShowRpcClient(TestWalletRpcClient):
        async def get_wallets(self, wallet_type: Optional[WalletType] = None) -> List[Dict[str, Union[str, int]]]:
            ...
            wallet_list: List[Dict[str, Union[str, int]]] = [
                {"data": "", "id": 1, "name": "NChain Wallet", "type": WalletType.STANDARD_WALLET},
                {
    ...
    assert_list = [
        "NChain Wallet:\n   -Total Balance:         0.001 nch (1000000000 mojo)",
...
def test_send(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    # get output with all options but verbose
    addr = encode_puzzle_hash(get_bytes32(3), "nch")
    command_args = [
        "wallet",
        "send",
        "-a1",
    ...
    expected_calls: logType = {
        ...
        "send_transaction": [
            (
                1,
                1000000000000,
                "nch1qvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvps3777dw",
                TXConfig(
...
def test_get_address(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class GetAddressWalletRpcClient(TestWalletRpcClient):
        async def get_next_address(self, wallet_id: int, new_address: bool) -> str:
            ...
            if new_address:
                return encode_puzzle_hash(get_bytes32(3), "nch")
            return encode_puzzle_hash(get_bytes32(4), "nch")

    inst_rpc_client = GetAddressWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    # get output with all options but verbose
    addr1 = encode_puzzle_hash(get_bytes32(3), "nch")
    addr2 = encode_puzzle_hash(get_bytes32(4), "nch")
    command_args = [
        "wallet",
        "get_address",
        WALLET_ID_ARG,
            ...
            return {
                ...
                "transactions": [
                    STD_TX.to_json_dict_convenience(
                        {
                            "selected_network": "ext9",
                            "network_overrides": {"config": {"ext9": {"address_prefix": "nch"}}},
                        }
                    )
                ],
            }
...
def test_sign_message(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    inst_rpc_client = TestWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    nch_addr = encode_puzzle_hash(get_bytes32(1), "nch")
    ...
    run_cli_command_and_assert(capsys, root_dir, command_args + [f"-a{nch_addr}"], assert_list)
    expected_calls: logType = {
        "sign_message_by_address": [(nch_addr, message.hex())],  # nch std
    }
    test_rpc_clients.wallet_rpc_client.check_log(expected_calls)
...
def test_make_offer_bad_filename(
    ...
    # we offer nch and a random cat via wallet id and request a random cat, nft via coin and tail
    command_args_dir = [
        "wallet",
        "make_offer",
        FINGERPRINT_ARG,
    ...
    expected_calls: logType = {
        ...
        "nft_calculate_royalties": [
            (
                ...
                {"NCH": 10000000000000, "test3": 100000},
            )
        ],
        "create_offer_for_ids": [
            (
...
def test_take_offer(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        "  OFFERED:\n"
        "    - NCH (Wallet ID: 1): 10.0 (10000000000000 mojos)\n"

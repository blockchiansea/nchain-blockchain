...
burn_address = encode_puzzle_hash(burn_ph, "nch")
burn_address_tnch = encode_puzzle_hash(burn_ph, "tnch")
burn_nft_addr = encode_puzzle_hash(burn_ph, "did:nchain:")
...
def test_click_amount_type() -> None:
    ...
    # Test Decimal / NCH (we don't test overflow because we don't know the conversion ratio yet)
...
def test_click_address_type() -> None:
    context = cast(Context, FakeContext(obj={"expected_prefix": "nch"}))  # this makes us not have to use a config file
    std_cli_address = CliAddress(burn_ph, burn_address, AddressType.NCH)
    ...
    assert std_cli_address.validate_address_type(AddressType.NCH) == burn_address
    assert std_cli_address.validate_address_type_get_ph(AddressType.NCH) == burn_ph
    ...
    with pytest.raises(AttributeError):  # attribute error because the context does not have a real error handler
        AddressParamType().convert(burn_address_tnch, None, context)
    with pytest.raises(BadParameter):
...
def test_click_address_type_config(root_path_populated_with_config: Path) -> None:
    ...
    assert AddressParamType().convert(burn_address, None, context) == CliAddress(burn_ph, burn_address, AddressType.NCH)
    assert context.obj["expected_prefix"] == "nch"  # validate that the prefix was set correctly
    # use tnch address
    with pytest.raises(AttributeError):  # attribute error because the context does not have a real error handler
        AddressParamType().convert(burn_address_tnch, None, context)

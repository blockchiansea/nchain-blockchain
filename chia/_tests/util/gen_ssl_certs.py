...
from chia.ssl.create_ssl import generate_ca_signed_cert, get_nchain_ca_crt_key, make_ca_cert
...
def gen_ssl(suffix: str = "") -> None:
    ...
    for node_name, cert_type_dict in node_certs_and_keys.items():
        for cert_type, cert_dict in cert_type_dict.items():
            crt = cert_dict["crt"]
            key = cert_dict["key"]
            ca_crt = nchain_ca_crt if cert_type == "public" else private_ca_crt
            ca_key = nchain_ca_key if cert_type == "public" else private_ca_key

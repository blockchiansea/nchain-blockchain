...
from chia.server.ssl_context import nchain_ssl_ca_paths, private_ssl_ca_paths
...
class TestSSL:
    ...
    async def test_farmer_wrong_ca(self, farmer_one_harvester, self_hostname):
        ...
        farmer_server = farmer_api.farmer.server
        nchain_ca_crt_path, nchain_ca_key_path = nchain_ssl_ca_paths(bt.root_path, bt.config)
        ...
        generate_ca_signed_cert(nchain_ca_crt_path.read_bytes(), nchain_ca_key_path.read_bytes(), pub_crt, pub_key)
        ssl_context = ssl_context_for_client(nchain_ca_crt_path, nchain_ca_key_path, pub_crt, pub_key)
    ...
    async def test_full_node(self, simulator_and_wallet, self_hostname):
        ...
        generate_ca_signed_cert(
            nchain_ca_crt_path.read_bytes(),
            nchain_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(nchain_ca_crt_path, nchain_ca_key_path, pub_crt, pub_key)
    ...
    async def test_introducer(self, introducer_service, self_hostname):
        ...
        nchain_ca_crt_path, nchain_ca_key_path = nchain_ssl_ca_paths(introducer_service.root_path, introducer_service.config)

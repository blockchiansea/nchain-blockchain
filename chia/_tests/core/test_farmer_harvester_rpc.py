...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment: HarvesterFarmerEnvironment) -> None:
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "nch"), encode_puzzle_hash(new_ph_2, "nch"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "nch"), encode_puzzle_hash(observer_pool, "nch")
    )
    ...
    assert config["farmer"]["nch_target_address"] == encode_puzzle_hash(observer_farmer, "nch")
    assert config["pool"]["nch_target_address"] == encode_puzzle_hash(observer_pool, "nch")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "nch")

...
class ChiaPlottersBladebitArgsCase:
    ...
    count: int = 1
    threads: int = 0
    pool_contract: str = "tnch1xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
    compress: int = 1
    device: int = 0
    ...
    async with aiohttp.ClientSession() as client:
        async with client.ws_connect(
            ...
            async with setup_full_node(
                ...
                await time_out_assert_not_none(30, ws_server.connections.get, "nchain_full_node")

                payload = create_payload("get_blockchain_state", {}, service_name, "nchain_full_node")
                await ws.send_str(payload)

                response = await ws.receive()
                message = assert_response_success_only(response)
                assert message["command"] == "get_blockchain_state"
                assert message["origin"] == "nchain_full_node"
        ...
        response={
            ...
            "wallet_addresses": {
                G1Element().get_fingerprint(): [
                    {
                        "address": "nch1dr2sj4jqdt6nj4l32d4f5dk7mrwak3qw5hsykty5lhhd00053y0s5f23vk",
                        "hd_path": "m/12381/8444/2/0",
                    }
                ],
                test_key_data.fingerprint: [
                    {
                        "address": "nch1zze67l3jgxuvyaxhjhu7326sezxxve7lgzvq0497ddggzhff7c9su49wqn",
                        "hd_path": "m/12381/8444/2/0",
                    },
                ],
                test_key_data_2.fingerprint: [
                    {
                        "address": "nch1fra5h0qnsezrxenjyslyxx7y4l268gq52m0rgenh58vn8f577uzsckyqmg",
                        "hd_path": "m/12381/8444/2/0",
                    }
                ],
            },
    ...
    WalletAddressCase(
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "nch16jqcaguq27z8xvpu89j7eaqfzn6k89hdrrlm0rffku85n8n7m7sqk0nten",
                        "hd_path": "m/12381/8444/2/1",
                    },
                    {
                        "address": "nch1955vj0gx5tqe7v5tceajn2p4z4pup8d4g2exs0cz4xjqses8ru6q2n2hlq",
    ...
    WalletAddressCase(
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "nch1k996a7h3agygjhqtrf0ycpa7wfd6k5ye2plkf54ukcmdj44gkqkq3n8fsh",
    ...
    RouteCase(
        ...
        response={
            ...
            "plotters": {
                ...
                "chiapos": {"display_name": "NChain Proof of Space", "installed": True, "version": chiapos_version},

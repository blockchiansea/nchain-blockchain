...
class TestConfig:
    ...
    def test_load_config(self, root_path_populated_with_config, default_config_dict):
        ...
        assert (
            config["farmer"]["network_overrides"]["constants"]["ext9"]["GENESIS_CHALLENGE"]
            == default_config_dict["farmer"]["network_overrides"]["constants"]["ext9"]["GENESIS_CHALLENGE"]
    ...
    def test_selected_network_address_prefix_default_config(self, config_with_address_prefix: Dict[str, Any]) -> None:
        """
        Temp config.yaml created using a default config. address_prefix is defaulted to "nch"
        """
    ...
    def test_selected_network_address_prefix_testnet_config(self, config_with_address_prefix: Dict[str, Any]) -> None:
        """
        Temp config.yaml created using a modified config. address_prefix is set to "tnch"
        """
        config = config_with_address_prefix
        prefix = selected_network_address_prefix(config)
        assert prefix == "tnch"
    ...
    def test_selected_network_address_prefix_config_dict(self, default_config_dict: Dict[str, Any]) -> None:
        """
        Modified config dictionary has address_prefix set to "custom nch"
        """
        config = default_config_dict
        config["network_overrides"]["config"][config["selected_network"]]["address_prefix"] = "customnch"
        prefix = selected_network_address_prefix(config)
        assert prefix == "customnch"

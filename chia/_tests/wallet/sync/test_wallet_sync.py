...
6. Clear all coins from the dust wallet.
    Send to the dust wallet "spam_filter_after_n_txs" coins that are equal in value to "nch_spam_amount".
    Send 1 mojo from the dust wallet. The dust wallet should receive a change coin valued at "nch_spam_amount-1".
...
@pytest.mark.anyio
@pytest.mark.parametrize(
    "spam_filter_after_n_txs, nch_spam_amount, dust_value",
    [
...
async def test_dusted_wallet(
    ...
    # It's also possible to obtain the current settings for spam_filter_after_n_txs and nch_spam_amount
    # spam_filter_after_n_txs = wallets[0][0].config["spam_filter_after_n_txs"]
    # nch_spam_amount = wallets[0][0].config["nch_spam_amount"]
    # dust_value=1
    ...
    # Current max value for nch_spam_amount is 0.01 NCH.
    ...
    assert nch_spam_amount >= 1
    assert nch_spam_amount <= 10_000_000_000
    assert dust_value >= 1
    ...
    # The dust is only filtered at this point if spam_filter_after_n_txs is 0 and nch_spam_amount is > dust_value.
    if spam_filter_after_n_txs > 0:
        dust_coins = 1
        large_dust_coins = 0
        large_dust_balance = 0
    elif nch_spam_amount <= dust_value:
        dust_coins = 0
        large_dust_coins = 1
    ...
    small_unspent_count = len([r for r in all_unspent if r.coin.amount < nch_spam_amount])
    ...
    log.info(f"nch_spam_amount {nch_spam_amount}")
    ...
    for _ in range(large_coins):
        payee_ph = await dust_wallet.get_new_puzzlehash()
        payees.append(Payment(payee_ph, uint64(nch_spam_amount)))
    ...
    large_coin_balance = large_coins * nch_spam_amount

    # Determine whether the filter should have been activated.
    # Make sure the number of coins matches the expected number.
    # At this point, nothing should be getting filtered unless spam_filter_after_n_txs is 0.
    assert dust_coins == spam_filter_after_n_txs
    assert balance == dust_coins * dust_value + large_coins * nch_spam_amount + large_dust_balance
    ...
    for i in range(5):
        ...
        payees.append(Payment(payee_ph, uint64(nch_spam_amount + i)))
        large_coins += 1
        large_coin_balance += nch_spam_amount + i

        payee_ph = await dust_wallet.get_new_puzzlehash()

        # Make sure we are always creating coins with a positive value.
        if nch_spam_amount - dust_value - i > 0:
            payees.append(Payment(payee_ph, uint64(nch_spam_amount - dust_value - i)))
        else:
            payees.append(Payment(payee_ph, uint64(dust_value)))
        # In cases where nch_spam_amount is sufficiently low,
        # the new dust should be considered a large coina and not be filtered.
        if nch_spam_amount <= dust_value:
            large_dust_coins += 1
            large_dust_balance += dust_value
    ...
    # The size of the coin to send the dust wallet is the same as nch_spam_amount
    if nch_spam_amount > 1:
        coin_value = nch_spam_amount
    else:

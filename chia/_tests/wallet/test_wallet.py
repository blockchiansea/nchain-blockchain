...
class TestWalletSimulator:
    ...
    async def test_sign_message(self, wallet_environments: WalletTestFramework) -> None:
        ...
        # Test general string
        message = "Hello World"
        ph = await env.nch_wallet.get_puzzle_hash(False)
        response = await api_0.sign_message_by_address({"address": encode_puzzle_hash(ph, "nch"), "message": message})
        ...
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "nch"), "message": message, "is_hex": True}
        )
        ...
        message = "0123456789ABCDEF"
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "nch"), "message": message, "is_hex": "true", "safe_mode": "true"}
        )
        puzzle = Program.to((CHIP_0002_SIGN_MESSAGE_PREFIX, bytes.fromhex(message)))
        ...
        message = "Hello World"
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "nch"), "message": message, "is_hex": False, "safe_mode": False}
        )
        ...
        message = "0123456789ABCDEF"
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "nch"), "message": message, "is_hex": True, "safe_mode": False}
        )
...
def test_get_wallet_db_path_v2_r1() -> None:
    root_path: Path = Path("/x/y/z/.nchain/ext9").resolve()
    config: Dict[str, Any] = {
    ...
    assert wallet_db_path == root_path.joinpath("wallet/db/blockchain_wallet_v2_r1_ext9_1234567890.sqlite")
...
def test_get_wallet_db_path_testnet() -> None:
    root_path: Path = Path("/x/y/z/.nchain/testnet").resolve()
    config: Dict[str, Any] = {
        "database_path": "wallet/db/blockchain_wallet_v2_CHALLENGE_KEY.sqlite",

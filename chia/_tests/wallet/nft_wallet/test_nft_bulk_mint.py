...
async def test_nft_mint_from_did(
    ...
    expected_nch_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_nch_bal)
...
async def test_nft_mint_from_did_rpc(
    ...
    try:
        ...
        target_list = [encode_puzzle_hash((ph_taker), "nch") for x in range(n)]
        royalty_address = encode_puzzle_hash(bytes32.random(seeded_random), "nch")
        royalty_percentage = 300
        fee = 100
        required_amount = n + (fee * n)
        nch_coins = await client.select_coins(
            amount=required_amount, coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG, wallet_id=wallet_maker.id()
        )
        funding_coin = nch_coins[0]
        assert funding_coin.amount >= required_amount
        funding_coin_dict = nch_coins[0].to_json_dict()
        chunk = 5
        ...
        for i in range(0, n, chunk):
            ...
            resp = await client.nft_mint_bulk(
                ...
                mint_total=n,
                nch_coins=[next_coin.to_json_dict()],
                nch_change_target=funding_coin_dict["puzzle_hash"],
            ...
            spends.append(sb)
            nch_adds = [c for c in sb.additions() if c.puzzle_hash == funding_coin.puzzle_hash]
            assert len(nch_adds) == 1
            next_coin = nch_adds[0]
...
@pytest.mark.anyio
async def test_nft_mint_from_did_multiple_nch(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    ...
    # Grab two coins for testing that we can create a bulk minting with more than 1 nch coin
    async with wallet_maker.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=False) as action_scope:
        nch_coins_1 = await wallet_maker.select_coins(amount=10000, action_scope=action_scope)
        nch_coins_2 = await wallet_maker.select_coins(
            amount=10000,
            action_scope=action_scope,
        )
    nch_coins = nch_coins_1.union(nch_coins_2)
    ...
    # confirm that the spend uses the right amount of nch
    expected_nch_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_maker.get_confirmed_balance, expected_nch_bal)
...
@pytest.mark.anyio
async def test_nft_mint_from_nch(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    ...
    async with nft_wallet_maker.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await nft_wallet_maker.mint_from_nch(
            metadata_list,
            action_scope,
            target_list=target_list,
            mint_number_start=1,
...
@pytest.mark.anyio
async def test_nft_mint_from_nch_rpc(
    two_wallet_nodes_services: SimulatorsAndWalletsServices,
    trusted: Any,
    self_hostname: str,
    seeded_random: random.Random,
...
@pytest.mark.anyio
async def test_nft_mint_from_nch_multiple_nch(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]

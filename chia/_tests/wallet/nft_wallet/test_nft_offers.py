...
async def test_nft_offer_with_fee(
    ...
    # MAKE FIRST TRADE: 1 NFT for 100 nch
    ...
    nch_request = 100
    maker_fee = uint64(10)
    offer_nft_for_nch = {wallet_maker.id(): nch_request, nft_asset_id: -1}
    maker_unused_index = (
    ...
    async with trade_manager_maker.wallet_state_manager.new_action_scope(tx_config, push=False) as action_scope:
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer_nft_for_nch, action_scope, driver_dict, fee=maker_fee
        )
    ...
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre + nch_request - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre - nch_request - taker_fee)
    ...
    # MAKE SECOND TRADE: 100 nch for 1 NFT
    ...
    nch_offered = 1000
    maker_fee = uint64(10)
    offer_nch_for_nft = {wallet_maker.id(): -nch_offered, nft_to_buy_asset_id: 1}

    async with trade_manager_maker.wallet_state_manager.new_action_scope(tx_config, push=False) as action_scope:
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer_nch_for_nft, action_scope, driver_dict_to_buy, fee=maker_fee
        )
    assert success is True
    assert error is None
    assert trade_make is not None
    ...
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre - nch_offered - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre + nch_offered - taker_fee)
...
@pytest.mark.parametrize("reuse_puzhash", [True, False])
@pytest.mark.anyio
async def test_nft_offer_nft0_and_nch_for_cat(
    self_hostname: str,
    two_wallet_nodes: Any,
    trusted: Any,
    reuse_puzhash: bool,
    ...
    maker_fee = uint64(10)
    maker_nch_offered = 1000
    taker_cat_offered = 2500
    wallet_maker_id = wallet_maker.id()
    offer_nft_for_cat = {
        wallet_maker_id: -maker_nch_offered,
        nft_asset_id: -1,
    ...
    maker_balance_post = await wallet_maker.get_confirmed_balance()
    taker_balance_post = await wallet_taker.get_confirmed_balance()
    assert maker_balance_post == maker_balance_pre - maker_fee - maker_nch_offered
    assert taker_balance_post == taker_balance_pre - taker_fee + maker_nch_offered
    coins_taker = await nft_wallet_taker.get_current_nfts()
    assert len(coins_taker) == 1

...
async def test_send_transaction(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(await wallet_2.get_new_puzzlehash(), "tnch")
...
async def test_spend_clawback_coins(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = (
        await wallet_1_rpc.send_transaction(
            wallet_id=1,
            amount=uint64(500),
            address=encode_puzzle_hash(wallet_2_puzhash, "tnch"),
            tx_config=DEFAULT_TX_CONFIG,
            fee=uint64(0),
    ...
    tx = (
        await wallet_2_rpc.send_transaction(
            wallet_id=1,
            amount=uint64(500),
            address=encode_puzzle_hash(wallet_1_puzhash, "tnch"),
            tx_config=DEFAULT_TX_CONFIG,
            fee=uint64(0),
            puzzle_decorator_override=[{"decorator": "CLAWBACK", "clawback_timelock": 5}],
        )
...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    await client.send_transaction(
        1, uint64(1), encode_puzzle_hash(puzhash, "tnch"), DEFAULT_TX_CONFIG
    )  # Create a pending tx
    ...
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    await client.send_transaction(1, uint64(1), encode_puzzle_hash(ph_by_addr, "tnch"), DEFAULT_TX_CONFIG)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    tx_for_address = await client.get_transactions(1, to_address=encode_puzzle_hash(ph_by_addr, "tnch"))
...
async def test_get_coin_records_by_names(wallet_rpc_environment: WalletRpcTestEnvironment) -> None:
    ...
    address = encode_puzzle_hash(await env.wallet_1.wallet.get_new_puzzlehash(), "tnch")
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
...
async def _check_delete_key(
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["nch_target_address"] = encode_puzzle_hash(farmer_ph, "tnch")
        test_config["pool"]["nch_target_address"] = encode_puzzle_hash(pool_ph, "tnch")
...
async def test_key_and_address_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(ph, "tnch")
    tx_amount = uint64(15600000)
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["nch_target_address"] = ""
        save_config(wallet_node.root_path, "config.yaml", test_config)
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["nch_target_address"] = ""
        test_config["pool"]["nch_target_address"] = ""
        save_config(wallet_node.root_path, "config.yaml", test_config)
...
#
# Example 1:
# $ nchain keys generate
# $ nchain keys sign -d 'hello world' -t 'm/12381/8444/1/1'
#
# Example 2:
# $ nchain wallet get_address
# nch1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
# $ nchain wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a nch1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
#
@pytest.mark.parametrize(
    ...
    [
        # Valid signatures
        (
            # nchain keys sign -d "Let's eat, Grandma" -t "m/12381/8444/1/1"
            {
        ...
        (
            # nchain wallet sign_message -m $(echo -n 'Happy happy joy joy' | xxd -p)
            # -a nch1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
        ...
        (
            ...
            # -a nch1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "nch1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35stf66cn",
            },
            VerifySignatureResponse(isValid=True),
        ),
        (
            {
                ...
                "address": "nch1hh9phcc8tt703dla70qthlhrxswy88va04zvc7vd8cx2v6a5ywysann7jm",
            },
            VerifySignatureResponse(isValid=True),
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "nch1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gdsydmg9t",
            },
...
async def test_cat_spend_run_tail(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = (
        await client.cat_spend(
            ...
            amount=uint64(0),
            tx_config=DEFAULT_TX_CONFIG,
            inner_address=encode_puzzle_hash(our_ph, "tnch"),
...
async def test_split_coins(wallet_environments: WalletTestFramework) -> None:
    ...
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config) as action_scope:
        target_coin = list(await env.nch_wallet.select_coins(uint64(250_000_000_000), action_scope))[0]
        assert target_coin.amount == 250_000_000_000

    nch_request = SplitCoins(
        wallet_id=uint32(1),
        number_of_coins=uint16(100),
        amount_per_coin=uint64(100),
        target_coin_id=target_coin.name(),
        fee=uint64(1_000_000_000_000),  # 1 NCH
        push=True,
    )
    ...
    response = await env.rpc_client.split_coins(
        dataclasses.replace(nch_request, number_of_coins=uint16(0)),
        wallet_environments.tx_config,
    )
...
async def test_combine_coins(wallet_environments: WalletTestFramework) -> None:
    ...
    # Should have 4 coins, two 1.75 NCH, two 0.25 NCH
    ...
    fee = uint64(100)
    nch_combine_request = CombineCoins(
        wallet_id=uint32(1),
    ...
    with pytest.raises(ResponseFailureError, match="Cannot combine coins from non-fungible wallet types"):
        await env.rpc_client.combine_coins(
            dataclasses.replace(nch_combine_request, wallet_id=uint32(42)),
            wallet_environments.tx_config,
        )
    del env.wallet_state_manager.wallets[uint32(42)]
    ...
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        await cat_wallet.generate_signed_transaction(
            ...
            [await env.nch_wallet.get_puzzle_hash(new=action_scope.config.tx_config.reuse_puzhash)] * 3,
            action_scope,
        )
...
async def test_fee_bigger_than_selection_coin_combining(wallet_environments: WalletTestFramework) -> None:
    ...
    with pytest.raises(ResponseFailureError, match="without selecting more coins than specified: 3"):
        await env.rpc_client.combine_coins(
            dataclasses.replace(nch_combine_request, fee=uint64(2_250_000_000_000)),
            wallet_environments.tx_config,
        )

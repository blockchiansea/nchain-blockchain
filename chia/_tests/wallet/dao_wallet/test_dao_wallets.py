...
async def test_dao_creation(self_hostname: str, two_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    # Try to create a DAO with more CATs than nch balance
...
async def test_dao_funding(self_hostname: str, three_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    # Create funding spends for nch and cat
    nch_funds = uint64(500000)
    cat_funds = uint64(100000)
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.create_add_funds_to_treasury_spend(nch_funds, action_scope)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    await time_out_assert(20, dao_wallet_0.get_balance_by_asset_type, nch_funds)
    ...
    nch_proposal_inner = generate_simple_proposal_innerpuz(
        treasury_id,
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, dao_cat_0_bal)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    await time_out_assert(30, dao_wallet_0.get_balance_by_asset_type, nch_funds - 10000)
    await time_out_assert(30, dao_wallet_0.get_balance_by_asset_type, cat_funds, cat_id)
    await time_out_assert(30, dao_wallet_1.get_balance_by_asset_type, nch_funds - 10000)
...
async def test_dao_proposals(self_hostname: str, three_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    # Create funding spend so the treasury holds some NCH
    nch_funds = uint64(500000)
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.create_add_funds_to_treasury_spend(nch_funds, action_scope)
    await full_node_api.wait_transaction_records_entered_mempool(
    ...
    await time_out_assert(10, dao_wallet_0.get_balance_by_asset_type, nch_funds)
    await time_out_assert(10, dao_wallet_1.get_balance_by_asset_type, nch_funds)
    await time_out_assert(10, dao_wallet_2.get_balance_by_asset_type, nch_funds)

    # Create Proposals

    # Proposal 0: Spend nch to wallet_2.
    recipient_puzzle_hash = await wallet_2.get_new_puzzlehash()
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, dao_cat_0_bal, fee=base_fee)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    # Proposal 3: Spend nch to wallet_2 (this prop will close as failed)
    ...
    nch_proposal_inner = Program.to(["x"])
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, dao_cat_0_bal, fee=base_fee)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    ...
    await time_out_assert(
        20, dao_wallet_0.get_balance_by_asset_type, nch_funds - proposal_amount_1 + proposal_min_amt - 1
    )
    ...
    await time_out_assert(20, wallet_2.get_confirmed_balance, funds + proposal_amount_1)
    expected_balance = nch_funds - proposal_amount_1 + (3 * proposal_min_amt) - 3 - new_mint_amount
...
async def test_dao_rpc_api(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    expected_nch = funds - 1 - cat_amt - fee
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_nch)
    ...
    nch_funding_amt = 200000
    nch_tx = await api_0.dao_add_funds_to_treasury(
        dict(
            wallet_id=dao_wallet_0_id,
            amount=nch_funding_amt,
            funding_wallet_id=1,
        )
    )
    txs = [TransactionRecord.from_json_dict(cat_tx["tx"]), TransactionRecord.from_json_dict(nch_tx["tx"])]
    await full_node_api.wait_transaction_records_entered_mempool(records=txs, timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)

    expected_nch -= nch_funding_amt + new_cat_amt
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_nch)
    ...
    await rpc_state(
        20, api_0.dao_get_treasury_balance, [{"wallet_id": dao_wallet_0_id}], lambda x: x["balances"]["nch"]
    )
    balances = await api_0.dao_get_treasury_balance({"wallet_id": dao_wallet_0_id})
    assert balances["balances"]["nch"] == nch_funding_amt
    ...
    await api_0.cat_spend(
        {
            ...
            "amount": cat_amt // 2,
            "inner_address": encode_puzzle_hash(ph_1, "nch"),
            "fee": fee,
        }
    )
    txs = await wallet_0.wallet_state_manager.tx_store.get_all_unconfirmed()
    ...
    mint_proposal = await api_0.dao_create_proposal(
        {
            ...
            "proposal_type": "mint",
            "amount": uint64(10000),
            "cat_target_address": encode_puzzle_hash(ph_0, "nch"),
            "vote_amount": cat_amt // 2,
            "fee": fee,
        }
    )
...
async def test_dao_rpc_client(
    ...
    try:
        ...
        # fund treasury
        nch_funds = uint64(10000000000)
        await client_0.dao_add_funds_to_treasury(dao_id_0, 1, nch_funds, DEFAULT_TX_CONFIG)
        ...
        await rpc_state(20, client_0.dao_get_treasury_balance, [dao_id_0], lambda x: x["balances"]["nch"], nch_funds)
        ...
        await rpc_state(
            20, client_0.dao_get_treasury_balance, [dao_id_0], lambda x: x["balances"]["nch"], nch_funds - 1000
        )
        ...
        expected_nch = initial_funds - amount_of_cats - new_cat_amt - nch_funds - (2 * fee) - 2 - 9000
        await rpc_state(
            20, client_0.get_wallet_balance, [wallet_0.id()], lambda x: x["confirmed_wallet_balance"], expected_nch
        )
...
async def test_dao_complex_spends(
    ...
    try:
        ...
        nch_funds = uint64(10000000000)
        for _ in range(4):
            await client_0.dao_add_funds_to_treasury(dao_id_0, 1, uint64(nch_funds / 4), DEFAULT_TX_CONFIG)
        ...
        # Test proposal with multiple conditions and nch coins
        additions = [
            {"puzzle_hash": ph_0.hex(), "amount": nch_funds / 4},
            {"puzzle_hash": ph_1.hex(), "amount": nch_funds / 4},
        ]
        await client_0.dao_create_proposal(
            wallet_id=dao_id_0,
            proposal_type="spend",
        ...
        await rpc_state(20, client_0.dao_get_proposals, [dao_id_0], lambda x: x["proposals"][-1]["closed"], True)
        await rpc_state(20, client_1.dao_get_proposals, [dao_id_1], lambda x: x["proposals"][-1]["closed"], True)
        # check the nch is received and removed from treasury
        await rpc_state(
            20,
            client_1.get_wallet_balance,
            [wallet_1.id()],
            lambda x: x["confirmed_wallet_balance"],
            initial_funds + (nch_funds / 4),
        )
        await rpc_state(
            20,
            client_0.dao_get_treasury_balance,
        ...
        await rpc_state(
            ...
            [dao_id_0],
            lambda x: x["balances"]["nch"] + 1,  # add 1 so result isn't 0
            1,
        )
        await rpc_state(
            20,
...
async def test_dao_concurrency(self_hostname: str, three_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    # Create a proposal for nch spend
    recipient_puzzle_hash = await wallet_2.get_new_puzzlehash()
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, dao_cat_0_bal, uint64(1000))
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
...
async def test_dao_cat_exits(
    ...
    try:
        ...
        # fund treasury
        nch_funds = uint64(10000000000)
        funding_tx = await client_0.dao_add_funds_to_treasury(dao_id_0, 1, nch_funds, DEFAULT_TX_CONFIG)
        tx = funding_tx.tx
        await full_node_api.wait_transaction_records_entered_mempool(records=[tx], timeout=60)
        await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
        await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
...
async def test_dao_reorgs(self_hostname: str, two_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    # Check that the funding spend is recognized by both dao wallets
    await time_out_assert(20, dao_wallet_0.get_balance_by_asset_type, nch_funds)
    await time_out_assert(20, dao_wallet_1.get_balance_by_asset_type, nch_funds)
...
async def test_dao_votes(self_hostname: str, three_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, vote_1, fee=base_fee)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, vote_2, fee=base_fee)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    vote_5 = uint64(1)
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, vote_5, fee=base_fee)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, fee=base_fee)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
...
async def test_dao_resync(self_hostname: str, two_wallet_nodes: OldSimulatorsAndWallets, trusted: bool) -> None:
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope, uint64(10))
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    async with dao_wallet_0.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        await dao_wallet_0.generate_new_proposal(nch_proposal_inner, action_scope)
    await full_node_api.wait_transaction_records_entered_mempool(
        records=action_scope.side_effects.transactions, timeout=60
    )
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)

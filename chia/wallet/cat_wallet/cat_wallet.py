...
class CATWallet:
    ...
    async def create_tandem_nch_tx(
        self,
        fee: uint64,
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some NCH as part of this
            ...
            if first:
                ...
                if need_chia_transaction:
                    if fee > regular_chia_to_claim:
                        await self.create_tandem_nch_tx(
                            fee,
                        ...
                        nch_announcement = await self.create_tandem_nch_tx(
                            fee,
                            uint64(regular_chia_to_claim),
                            action_scope,
                        )
                        assert nch_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            conditions=(*extra_conditions, nch_announcement, announcement),
                        )
                    else:

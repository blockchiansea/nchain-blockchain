...
class NFTWallet:
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            await self.standard_wallet.create_tandem_nch_tx(
                fee,
                action_scope,
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'NCH'} to offer")
    ...
    async def mint_from_did(
        ...
        nch_coins: Optional[Set[Coin]] = None,
        nch_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an nch spend that funds the transactions and pays fees.
        ...
        :param nch_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param nch_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the nch spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an nch coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if nch_coins is None:
            nch_coins = await self.standard_wallet.select_coins(uint64(total_amount), action_scope)
        assert len(nch_coins) > 0
        ...
        # Create the nch spend to fund the minting.
        spend_value = sum(coin.amount for coin in nch_coins)
        change: uint64 = uint64(spend_value - total_amount)
        if nch_change_ph is None:
            nch_change_ph = await self.standard_wallet.get_new_puzzlehash()
        nch_payment = Payment(nch_change_ph, change, [nch_change_ph])

        nch_coins_iter = iter(nch_coins)
        nch_coin = next(nch_coins_iter)

        message_list: List[bytes32] = [c.name() for c in nch_coins]
        message_list.append(Coin(nch_coin.name(), nch_payment.puzzle_hash, nch_payment.amount).name())
        message: bytes32 = std_hash(b"".join(message_list))

        nch_extra_conditions: Tuple[Condition, ...] = (
            AssertCoinAnnouncement(asserted_id=did_coin.name(), asserted_msg=message),
        )
        if len(nch_coins) > 1:
            nch_extra_conditions += (CreateCoinAnnouncement(message),)

        solution: Program = self.standard_wallet.make_solution(
            primaries=[nch_payment],
            fee=fee,
            conditions=nch_extra_conditions,
        )
        primary_announcement_hash = AssertCoinAnnouncement(asserted_id=nch_coin.name(), asserted_msg=message).msg_calc
        # connect this coin assertion to the DID announcement
        did_coin_announcement = CreateCoinAnnouncement(message)
        puzzle = await self.standard_wallet.puzzle_for_puzzle_hash(nch_coin.puzzle_hash)
        nch_spends = [make_spend(nch_coin, puzzle, solution)]

        for nch_coin in nch_coins_iter:
            puzzle = await self.standard_wallet.puzzle_for_puzzle_hash(nch_coin.puzzle_hash)
            solution = self.standard_wallet.make_solution(
                primaries=[], conditions=(AssertCoinAnnouncement(primary_announcement_hash),)
            )
            nch_spends.append(make_spend(nch_coin, puzzle, solution))
        nch_spend = WalletSpendBundle(nch_spends, G2Element())
        ...
        list_of_coinspends = [did_spend] + intermediate_coin_spends + launcher_spends + nch_spend.coin_spends
    ...
    async def mint_from_nch(
        ...
        """
        Minting NFTs from a single NCH spend using intermediate launcher puzzle
        :param metadata_list: A list of dicts containing the metadata for each NFT to be minted
        :param target_list: [Optional] a list of targets for transfering minted NFTs (aka airdrop)
        :param mint_number_start: [Optional] The starting point for mint number used in intermediate launcher
        puzzle. Default: 1
        ...
        funding_coin = nch_coins.copy().pop()
        ...
        # Empty set to load with the announcements we will assert from NCH to
        # match the announcements from the intermediate launcher puzzle
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # create an ASSERT_COIN_ANNOUNCEMENT for the NCH spend. The
            # intermediate launcher coin issues a CREATE_COIN_ANNOUNCEMENT of
            # the mint_number and mint_total for the launcher coin it creates
            intermediate_announcement_message = std_hash(int_to_bytes(mint_number) + int_to_bytes(mint_total))
            coin_announcements.add(std_hash(intermediate_launcher_coin.name() + intermediate_announcement_message))

            # Create the launcher coin, and add its id to a list to be asserted in the NCH spend
            ...
            # Annnouncements for eve spend. These are asserted by the nch spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        first = True
        for nch_coin in nch_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(nch_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in nch_coins]
                message_list.append(Coin(nch_coin.name(), nch_payment.puzzle_hash, nch_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[nch_payment] + primaries,
                    fee=fee,
                    conditions=extra_conditions,
                )
                primary_announcement = AssertCoinAnnouncement(asserted_id=nch_coin.name(), asserted_msg=message)
                first = False
            else:
        ...
        # Collect up all the coin spends and sign them
        list_of_coinspends = intermediate_coin_spends + launcher_spends + nch_spends
        unsigned_spend_bundle = WalletSpendBundle(list_of_coinspends, G2Element())

...
class WalletStateManager:
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the NCH coin
        ...
        if fee > 0:
            async with self.new_action_scope(action_scope.config.tx_config, push=False) as inner_action_scope:
                await self.main_wallet.create_tandem_nch_tx(
                    fee,
                    inner_action_scope,
                    extra_conditions=(
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        nch_spam_amount = self.config.get("nch_spam_amount", 1000000)
        ...
        small_unspent_count = await self.coin_store.count_small_unspent(nch_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < nch_spam_amount

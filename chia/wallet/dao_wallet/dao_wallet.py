...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive NCH though a wallet of this type.
    ...
    # if asset_id == None: then we get normal NCH
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSChiaConnection, coin_data: Optional[Any]) -> None:
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                nch_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == nch_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        # get existing nch funds for treasury
        nch_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([nch_funds_puzhash], [self.id()])
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    nch_coins = await self.select_coins_for_asset_type(uint64(sum), action_scope)
                    for nch_coin in nch_coins:
                        nch_parent_amount_list.append([nch_coin.parent_coin_info, nch_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(make_spend(nch_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            await self.standard_wallet.create_tandem_nch_tx(fee, action_scope)
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "nch_conditions": nch_created_coins,

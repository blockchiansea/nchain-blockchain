...
class TradeManager:
    ...
    By default, standard NCH is supported but to support other types of assets you must implement certain functions on
    ...
    async def cancel_pending_offers(
        ...
        for trade, cancellation_coins in zip(trade_records, all_cancellation_coins):
            ...
            for coin in cancellation_coins:
                ...
                # This should probably not switch on whether or not we're spending a NCH but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, amount in offer_dict.items():
                ...
                # asset_id can either be none if asset is NCH or
                ...
                if amount > 0:
                    ...
                    # For the NCH wallet also include the fee amount to the coins we use to pay this offer
                ...
                if asset_id is not None and wallet is not None:  # if this asset is not NCH
            ...
            # The access of the sorted keys here makes sure we create the NCH transaction first to make sure we pay fee
            # with the NCH side of the offer and don't create an extra fee transaction in other wallets.
            for id in sorted(coins_to_offer.keys(), key=lambda id: id != 1):
                ...
                async with self.wallet_state_manager.new_action_scope(
                    ...
                    # This should probably not switch on whether or not we're spending NCH but it has to for now
                    assert wallet is not None
                    if wallet.type() == WalletType.STANDARD_WALLET:
                        assert isinstance(wallet, Wallet)
                        await wallet.generate_signed_transaction(

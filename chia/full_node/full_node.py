...
class FullNode:
                    ...
                    async with conn.execute(
                        ...
                        if len(list(await cur.fetchall())) == 0:
                            try:
                                ...
                                # it could be a database created with "nchain init", which is
    ...
    def set_server(self, server: ChiaServer) -> None:
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.ext9.org")
        try:
    ...
    async def add_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'nch')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )

...
def get_nchain_ca_crt_key() -> Tuple[Any, Any]:
    here = importlib_resources.files(__name__.rpartition(".")[0])
    crt = here.joinpath("nchain_ca.crt").read_bytes()
    key = here.joinpath("nchain_ca.key").read_bytes()
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "NChain"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "NChain"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("ext9.org")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "NChain"),
            x509.NameAttribute(NameOID.COMMON_NAME, "NChain CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    nchain_ca_crt, nchain_ca_key = get_nchain_ca_crt_key()
    nchain_ca_crt_path = ca_dir / "nchain_ca.crt"
    nchain_ca_key_path = ca_dir / "nchain_ca.key"
    write_ssl_cert_and_key(nchain_ca_crt_path, nchain_ca_crt, nchain_ca_key_path, nchain_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./nchain_ca.crt"), Path("./nchain_ca.key"))
